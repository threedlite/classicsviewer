...
def get_section_line_mapping(cursor, book_id, max_section, segment_count=None):
    """Create a mapping from section numbers to line ranges with improved distribution"""
    ...
    # This catches cases like Aristotle's Poetics with 62 sections mapped to 613 lines
    if segment_count and abs(max_section - segment_count) <= 5:
        print(f"          Detected section-based numbering: {max_section} sections for {segment_count} segments across {max_line} lines")
        section_map = {}
    ...
    # Original logic but with better threshold
    # Only map if sections are numbered significantly lower than lines
    if max_section < max_line / 3 and max_section < 200:  # Avoid mapping large section numbers
        section_map = {}
        lines_per_section = max_line / max_section
...
def extract_translation_segments(book_elem, book_id, cursor, translator):
    """Extract translation segments based on milestone markers or structure"""
    segments = []
    ...
    # First check for chapter.subchapter structure (common in Aristotle, other philosophical texts)
    chapter_segments = []
    for chapter in book_elem.findall('.//div[@type="textpart"][@subtype="chapter"]'):
        ...
        if subchapters:
            for i, subchapter in enumerate(subchapters):
                ...
                text = get_text_content(subchapter).strip()
                if text and len(text) > 20:
                    # Use sequential numbering for segments
                    chapter_segments.append({
                        'seq_num': len(chapter_segments) + 1,
                        'text': text,
    ...
    if chapter_segments:
        print(f"          Found chapter.subchapter structure: {len(chapter_segments)} segments")
        # Convert to standard segment format using sequential numbering
        for seg in chapter_segments:
            segments.append({
                'start_line': seg['seq_num'],
                'end_line': seg['seq_num'],
                'text': seg['text'],
...
def process_perseus_author(author_dir, language, cursor):
    ...
    cursor.execute("INSERT OR IGNORE INTO authors VALUES (?, ?, ?, ?)",
                   (author_id, author_name, None, language))
