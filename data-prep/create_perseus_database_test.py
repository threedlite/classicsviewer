...
def extract_translation_segments(book_elem, book_id, cursor, translator):
    ...
    # Debug output
    if book_id.startswith('tlg0003'):
        print(f"          DEBUG: max_section={max_section}, mapping={'YES' if section_map else 'NO'}")
        if section_map and max_section > 0:
            print(f"          DEBUG: Example mappings: 1→{section_map.get(1)}, {max_section}→{section_map.get(max_section)}")
    ...
    for segment in segments:
        ...
        if section_map and start_line in section_map:
            mapped_start, mapped_end = section_map[start_line]
            if book_id.startswith('tlg0003') and start_line in [1, 146]:
                print(f"          DEBUG: Mapping section {start_line} to lines {mapped_start}-{mapped_end}")
            start_line, end_line = mapped_start, mapped_end
...
def create_database():
    ...
    # Define Greek authors with their TLG numbers
    greek_authors = {
        "tlg0003": "Thucydides"
    }
    
    # Process each Greek author
