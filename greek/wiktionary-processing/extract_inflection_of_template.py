...
def main():
    ...
    # Use dump file - this script requires the full dump
    dump_file = Path(__file__).parent.parent.parent / "data-sources" / "enwiktionary-latest-pages-articles.xml.bz2"
    ...
    # Extract from dump
    success = extract_inflection_of_mappings(str(dump_file), str(output_file))
    if not success:
