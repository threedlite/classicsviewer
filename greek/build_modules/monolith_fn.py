...
# Canonical schema lives at repo root; greek/build_modules is 3 levels deep.
_REPO_ROOT = Path(__file__).resolve().parent.parent.parent
_GREEK_ROOT = Path(__file__).resolve().parent.parent
_BUILD_MODULES = Path(__file__).resolve().parent
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))
if str(_BUILD_MODULES) not in sys.path:
    sys.path.insert(0, str(_BUILD_MODULES))
from shared.database_schema import create_schema as _canonical_create_schema  # noqa: E402

# After the vendoring move, build_modules is the file's own directory.
# `from build_modules.X` style imports still work because we added the
# parent (greek/) to sys.path implicitly via _BUILD_MODULES.
from load_combined_dictionaries import load_combined_dictionaries  # noqa: E402
from normalization_utils import normalize_greek, normalize_greek_ultra  # noqa: E402
# Interlinear generation removed - always use pregenerated files from greek/interlinear_output/
...
def process_translations(work_dir, work_id, cursor, altbook_mapping=None):
    ...
    aligned_dir = _REPO_ROOT / "aligned"
...
def import_interlinear_translations(db_filename, work_ids=None, interlinear_dir=None, mode='full'):
    ...
    if interlinear_dir is None:
        interlinear_dir = _BUILD_MODULES / "generate_interlinear"
    else:
    ...
    for work_id, xml_file in interlinear_files.items():
        ...
        try:
            ...
            # Per CLAUDE.md: no silent failures on critical components.
            # A corrupt/truncated interlinear XML means the generation step
            # was interrupted and its output is unshippable — the only valid
            # fix is to regenerate the entire interlinear corpus (no targeted
            # rebuilds; no manual patch-ups). Abort the build here so the
            # regression can't silently slip into a release.
            conn.rollback()
            conn.close()
            raise RuntimeError(
                f"CRITICAL: interlinear XML import failed for {xml_file.name}: {e}\n"
                f"  Path: {xml_file}\n"
                f"  Cause: the interlinear generator produced a corrupt or truncated XML\n"
                f"         (usually because a prior generation run was interrupted).\n"
                f"  Fix:   re-run the FULL interlinear generation from scratch:\n"
                f"           cd greek/build_modules/generate_interlinear\n"
                f"           ./run_interlinear_no_sleep.sh INTERLINEAR_ALL_GREEK_WITH_IDS.csv \\\n"
                f"               ../../greek_texts.db 8\n"
...
def create_database(mode='full', custom_csv_path=None, output_name=None):
    ...
    # Paths — post-vendoring, this module lives at greek/build_modules/.
    # Output DB still writes next to the module (greek/build_modules/...)
    # and the greek wrapper moves it to greek/greek_texts.db afterwards.
    ...
    if mode == 'sample':
        ...
        if custom_csv_path:
            ...
            sample_authors_file = _GREEK_ROOT / "data" / "SAMPLE_AUTHORS.csv"
    ...
    # Create all canonical tables + indexes (single source of truth).
    # shared/database_schema.py mirrors the shipped perseus_texts-2.db schema
    # byte-for-byte; Room validates against it on every app launch.
    print("Creating tables...")
    _canonical_create_schema(conn)
...
def import_lexicons_for_languages(db_filename, languages, lexicon_paths):
    ...
    for language in languages_to_import:
        ...
        if not os.path.exists(lexicon_zip):
            # Per CLAUDE.md no-silent-failure policy: a lexicon listed in
            # LEXICON_PATHS that isn't on disk is a build-setup error, not a
            # soft skip. Ship without it and dictionary lookups for that
            # language silently break.
            raise FileNotFoundError(
                f"CRITICAL: {language} lexicon missing: {lexicon_zip}\n"
                f"  Absolute path: {os.path.abspath(lexicon_zip)}\n"
                f"  Fix: build the {language} module's lexicon (see BUILD.md Step 6)\n"
                f"       before running assembly."
            )
...
if __name__ == "__main__":
    # create_perseus_database.py is a library, not an entry point. The
    # monolith-driven build has been retired — author processing, merging,
    # interlinear import, compression, and APK-asset copying are now
    # orchestrated by data-prep/assemble_database.py against per-language
    # module DBs (greek/, latin/, sanskrit/, ...).
    #
    # To build:
    #   cd greek && ./run_build.sh [sample|full|extended]
    #   cd latin && ./run_build.sh [sample|full|extended]
    #   python3 data-prep/assemble_database.py [sample|full|extended] [--skip-oga]
    #
    # See BUILD.md for the full pipeline.
    import sys
    print(
        "create_perseus_database.py is no longer an entry point.\n"
        "\n"
        "Build the language modules first:\n"
        "  cd greek && ./run_build.sh [sample|full|extended]\n"
        "  cd latin && ./run_build.sh [sample|full|extended]\n"
        "\n"
        "Then assemble the full DB:\n"
        "  python3 data-prep/assemble_database.py [sample|full|extended] [--skip-oga]\n"
        "\n"
        "See BUILD.md for the full pipeline.",
        file=sys.stderr,
