...
def main():
    ...
    for headword, data in wiktionary_data.items():
        ...
        # extract_wiktionary_final.py may have back-filled the definition from
        # LSJ/Cunliffe when Wiktionary had only a "Morphological entry" stub.
        # In that case it sets 'source' accordingly; we must propagate it so
        # the interlinear gloss extractor parses the body with the matching
        # format parser. Tagging Cunliffe-formatted text as 'wiktionary'
        # produces garbage like "(5)(11)]. 3 sing. aor. 124, 496, 201...".
        ...
        entry_key = (headword, effective_source)
        if entry_key not in seen_entries:
            seen_entries.add(entry_key)
            dictionary_entries_list.append({
                "headword": headword,
        ...
        # Always add self-mapping for Wiktionary entries (lemma_map source stays
        # 'wiktionary' since the mapping itself came from Wiktionary morphology,
        # even if the definition body came from LSJ/Cunliffe).
        mapping_key = (headword, headword)
        if mapping_key not in seen_mappings:
            seen_mappings.add(mapping_key)
            lemma_mappings.append({
