...
def _generate_work(work_id: str, output_dir: Path, no_tree: bool = False):
    ...
    # Generate output filenames. We write to `.tmp` siblings first and only
    # atomically rename on full successful completion. This protects against
    # truncated outputs when a worker process is killed mid-stream (SIGKILL,
    # OOM, Ctrl+C before Python can handle it, etc.) — the real XML either
    # stays untouched (if a previous valid version exists) or doesn't appear
    # at all. Without this, the import step silently ingests half-written
    # XMLs and ships a corrupt DB.
    ...
    txt_tmp = output_file.with_suffix(output_file.suffix + ".tmp")
    xml_tmp = xml_output_file.with_suffix(xml_output_file.suffix + ".tmp")

    # Clean up any .tmp left behind by a prior interrupted run before we
    # start a new attempt, so we never mix partial state.
    for stale in (txt_tmp, xml_tmp):
        if stale.exists():
            stale.unlink()
    ...
    try:
        ...
        with open(txt_tmp, 'w', encoding='utf-8') as txt_file, \
             open(xml_tmp, 'w', encoding='utf-8') as xml_file, \
        ...
        # Files are closed and flushed — safe to atomically rename into
        # place. os.replace is atomic on POSIX: the final XML either shows
        # up fully-formed or not at all. No partial/truncated state can
        # ever appear at the canonical path.
        import os as _os
        _os.replace(txt_tmp, output_file)
        _os.replace(xml_tmp, xml_output_file)
        ...
        # Clean up partial .tmp files on error. Leave the real final files
        # (output_file / xml_output_file) untouched — if they existed from a
        # prior successful run, we keep that valid output rather than
        # destroying it on a failed retry.
        for stale in (txt_tmp, xml_tmp):
            if stale.exists():
                stale.unlink()
        print(f"\n{'=' * 80}")
