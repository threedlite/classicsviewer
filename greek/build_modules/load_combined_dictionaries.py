...
def load_combined_dictionaries(cursor, build_mode='full', skip_latin=False):
    ...
    # Dictionary tables (dictionary_entries, lemma_map, normalization_patterns,
    # prefix_assimilation_rules) are created up front by shared.database_schema.
    # Clear prior rows so repeat runs within one build are idempotent.
    print("Clearing dictionary tables...")
    cursor.execute("DELETE FROM dictionary_entries")
    cursor.execute("DELETE FROM lemma_map")
    cursor.execute("DELETE FROM normalization_patterns")
    cursor.execute("DELETE FROM prefix_assimilation_rules")
    ...
    if skip_latin:
        ...
        try:
            # Dead path: Greek module always passes skip_latin=True. If a
            # future caller ever reaches this, load_whitakers_latin now lives
            # under latin/build_modules/ as the Latin module was extracted.
            from load_whitakers_latin import load_whitakers_latin  # noqa: F401
    ...
    # Post-vendoring: prefix-rule CSVs live with the language they belong to.
    # Greek rules: greek/data/greek_prefix_assimilation_rules.csv.
    # Latin rules: latin/data/latin_prefix_assimilation_rules.csv, loaded by
    # the Latin module; we skip them here when skip_latin=True.
    greek_data_dir = Path(__file__).parent.parent / "data"

    rules_imported = 0

    # Import Greek prefix assimilation rules
    greek_rules_file = greek_data_dir / "greek_prefix_assimilation_rules.csv"
    ...
    if skip_latin:
        ...
        # Latin rules live under latin/data/ since the Latin module owns them.
        latin_rules_file = (Path(__file__).parent.parent.parent
                            / "latin" / "data"
                            / "latin_prefix_assimilation_rules.csv")
