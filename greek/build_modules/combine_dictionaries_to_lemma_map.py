...
def combine_dictionaries():
    ...
    for headword, data in wiktionary_data.items():
        if headword not in dictionary_entries:  # Don't override Cunliffe or LSJ
            ...
            # Track the real source of the definition. If we fall back to LSJ
            # or Cunliffe below, the interlinear gloss extractor needs to know
            # which source-specific parser to use — tagging Cunliffe text as
            # 'wiktionary' causes _extract_wiktionary_gloss to emit garbage
            # like "(5)(11)]. 3 sing. aor. 124, 496, 201..." (see ἐφίστημι).
            # extract_wiktionary_final.py may already have back-filled the
            # definition from LSJ/Cunliffe and set 'source' accordingly.
            effective_source = data.get("source", "wiktionary")
            ...
            if "Morphological entry" in definition:
                ...
                for candidate in candidates:
                    ...
                    if candidate in dictionary_entries:
                        ...
                        # Inherit the real source so downstream parsers pick
                        # the right extractor (lsj / cunliffe / wiktionary).
                        effective_source = existing_entry.get("source", effective_source)
