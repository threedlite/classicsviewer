...
object DatabaseValidator {
    ...
    fun validateDatabase(context: Context, dbFile: File): ValidationResult {
        try {
            ...
            android.util.Log.e("DatabaseValidator", "Validation failed", e)
            return ValidationResult(false, "Invalid SQLite database: ${e.message}")
        }
    }
    
    private fun compareSchemas(externalDb: SQLiteDatabase, bundledDb: SQLiteDatabase, externalPath: String, bundledPath: String): ValidationResult {
        ...
        android.util.Log.d("DatabaseValidator", "Bundled database tables (${bundledTables.size}): ${bundledTables.joinToString(", ")}")
        android.util.Log.d("DatabaseValidator", "External database tables (${externalTables.size}): ${externalTables.joinToString(", ")}")
        
        // Check each bundled table exists in external database
        for (tableName in bundledTables) {
            if (tableName !in externalTables) {
                val error = "Table '$tableName' missing from external database"
                android.util.Log.e("DatabaseValidator", error)
                ...
                if (schemaErrors.isNotEmpty()) {
                    android.util.Log.e("DatabaseValidator", "Schema errors for table '$tableName':")
        ...
        return when {
            missingTables.isNotEmpty() -> {
                val errorMsg = "Missing required tables: ${missingTables.joinToString(", ")}\n" +
                              "Comparing:\n  External: $externalPath\n  Bundled: $bundledPath"
                android.util.Log.e("DatabaseValidator", errorMsg)
                ValidationResult(false, errorMsg, missingTables)
            }
            schemaMismatches.isNotEmpty() -> {
                val errorMsg = "Schema mismatches found:\n${schemaMismatches.joinToString("\n")}\n" +
                              "Comparing:\n  External: $externalPath\n  Bundled: $bundledPath"
                android.util.Log.e("DatabaseValidator", errorMsg)
                ValidationResult(false, errorMsg, schemaMismatches = schemaMismatches)
            }
            else -> {
                android.util.Log.d("DatabaseValidator", "Database validation successful!")
