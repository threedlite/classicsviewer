...
interface TextLineDao {
    @Query("SELECT * FROM text_lines WHERE book_id = :bookId ORDER BY line_number, sequence_number")
    suspend fun getByBook(bookId: String): List<TextLineEntity>
    
    @Query("SELECT * FROM text_lines WHERE book_id = :bookId AND line_number >= :startLine AND line_number <= :endLine ORDER BY line_number, sequence_number")
    suspend fun getByBookAndRange(bookId: String, startLine: Int, endLine: Int): List<TextLineEntity>
    
    @Query("SELECT * FROM text_lines WHERE book_id = :bookId AND line_number = :lineNumber AND sequence_number = :sequenceNumber")
    suspend fun getByBookLineAndSequence(bookId: String, lineNumber: Int, sequenceNumber: Int): TextLineEntity?
