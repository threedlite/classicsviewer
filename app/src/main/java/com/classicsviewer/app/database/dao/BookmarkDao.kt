...
interface BookmarkDao {
    ...
    @Query("SELECT * FROM bookmarks WHERE book_id = :bookId AND line_number = :lineNumber AND sequence_number = :sequenceNumber LIMIT 1")
    suspend fun getBookmark(bookId: String, lineNumber: Int, sequenceNumber: Int): BookmarkEntity?
    
    @Query("SELECT * FROM bookmarks WHERE id = :bookmarkId LIMIT 1")
    suspend fun getBookmarkById(bookmarkId: Long): BookmarkEntity?
    
    @Query("SELECT EXISTS(SELECT 1 FROM bookmarks WHERE book_id = :bookId AND line_number = :lineNumber AND sequence_number = :sequenceNumber)")
    suspend fun isBookmarked(bookId: String, lineNumber: Int, sequenceNumber: Int): Boolean
    ...
    @Query("DELETE FROM bookmarks WHERE book_id = :bookId AND line_number = :lineNumber AND sequence_number = :sequenceNumber")
    suspend fun deleteBookmarkByLocation(bookId: String, lineNumber: Int, sequenceNumber: Int)
