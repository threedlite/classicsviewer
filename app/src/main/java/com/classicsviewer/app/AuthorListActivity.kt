...
class AuthorListActivity : BaseActivity() {
    ...
    private var adapter: FilterableAuthorAdapter? = null
    ...
    private fun loadAuthors(language: String) {
        lifecycleScope.launch {
            ...
            adapter = FilterableAuthorAdapter(authors, PreferencesManager.getInvertColors(this@AuthorListActivity)) { author ->
    ...
    private fun updateResultCount() {
        adapter?.let {
            ...
            if (filtered < total) {
                binding.resultCountText.text = "Showing $filtered of $total authors"
